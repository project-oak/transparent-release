...
by the Transparent Release project for specifying fuzzing claims is described.
...
- **evidence** (array of objects, required): as defined by the [Claim Format](claim-transparency.md#fields).
    It is the collection of the fuzzing reports that are used to generate the FuzzClaim.

### FuzzClaim specification

In this section, the metrics/statistics that are included in the `claimSpec` section of the FuzzClaim
...
The metric that is used in the fuzzing claims to characterize crashes is **whether a crash is detected**.
...
In our use case, we target metrics that are fine-grained enough to provide reasonable indicators of the
...
of code gives the illusion that most of the code was executed. Therefore, it should be coupled with
line coverage.

The rest of the coverage metrics (function coverage, instantiation coverage, and region coverage) are not
used by FuzzBinder since they are less informative about the covered code and harder to analyze or
...
FuzzBinder allows the trusted party who will sign the FuzzClaim (a person, a team, or a trusted
tool) to decide whether to sign and publish the claim based on the generated FuzzClaim specifications. In
particular, it is up to the trusted party to decide to publish a FuzzClaim when certain crashes are
...
- Note that the coverage reports in OSS-Fuzz and ClusterFuzz GCS buckets are organized by date.
  Therefore, FuzzBinder needs to link the fuzzing date to the revision of the code that was used to generate them
  (the coverage reports) to extract the fuzzing data of that revision of the code.
  To link the date to the revision of the source code, two solutions can be considered. The first is to
  extract the revision hash from the coverage builds metadata and the second is to extract it from the fuzzers
  ...
  build uses exactly one revision of the source code (the same for all fuzzers).
  ...
  consistency can be checked by verifying that there is fuzzing efforts for all the fuzz-targets.

- Note that the extracted information from fuzzing logs needs to be aggregated to get the total fuzzing
  effort per target or per project on a given day.

In the table below, the source from which each metric/statistic can be extracted is provided. These
metrics are used to generate the claimSpec of the FuzzClaims.
...
The evidence is the collection of the fuzzing reports used to generate a FuzzClaim on a
given date, and help to reproduce the claim in the future if verification is needed. In FuzzBinder, the
evidence roles are:
...
- **“srcmap”**: refers to the role of the evidence files that help to map a given date to a revision of
  ...
  `gs://oss-fuzz-coverage/{projectName}/srcmap/{date}.json` for this purpose since they guarantee that 
  each date is linked to exactly one revision of the source code.
...
- Note that the URI of the evidence (used in the current implementation of FuzzBinder on GitHub) is based
  ...
  (which are important for reproducing the claims) are included in the URI of the evidence. 
...
As mentioned above, the used evidence files are currently stored in Google cloud Storage by OSS-Fuzz.
...
GCS bucket of OSS-Fuzz or ClusterFuzz to [Ent](https://github.com/google/ent) and generates the claims. 
If the original evidence is not available in the GCS bucket of OSS-Fuzz, FuzzBinder does not generate the claims.
...
- FuzzBinder will be wrapped in a [GitHub workflow](https://docs.github.com/en/actions/using-workflows/about-workflows).
  ...
  FuzzBinder), and an [in-toto run action](https://github.com/marketplace/actions/in-toto-run)
...
- **Fuzzing**: an automated testing technique that allows vulnerability detection by generating malformed
...
- **Instantiation coverage**: the percentage of function instantiations which have been executed at least
  ...
  have multiple instantiations. [[source](https://clang.llvm.org/docs/SourceBasedCodeCoverage.html#interpreting-reports)]
