...
func generateFuzzClaimSpec(c *gcsutil.Client, revisionDigest intoto.DigestSet, fuzzParameters *FuzzParameters, fuzzTargets []string) (*FuzzClaimSpec, error) {
	...
	for _, fuzzTarget := range fuzzTargets {
		coverage, err := GetCoverage(c, fuzzParameters, fuzzTarget, "perTarget")
		if err != nil {
			return nil, fmt.Errorf(
				"could not get %s coverage to generate the fuzzing ClaimSpec: %v", fuzzTarget, err)
		}
		fuzzEffort, err := GetFuzzEffort(c, revisionDigest, fuzzParameters, fuzzTarget)
		if err != nil {
			return nil, fmt.Errorf(
				"could not get %s fuzzing efforts to generate the fuzzing ClaimSpec: %v", fuzzTarget, err)
		}
		crash, err := GetCrashes(c, revisionDigest, fuzzParameters, fuzzTarget)
		if err != nil {
			return nil, fmt.Errorf(
	...
	projectCoverage, err := GetCoverage(c, fuzzParameters, "", "perProject")
	if err != nil {
		return nil, fmt.Errorf(
...
func GenerateFuzzClaim(c *gcsutil.Client, fuzzParameters *FuzzParameters, validity amber.ClaimValidity) (*intoto.Statement, error) {
	revisionDigest, err := GetCoverageRevision(c, fuzzParameters)

	if err != nil {
		return nil, fmt.Errorf(
			"could not get the revision digest to generate the fuzzing claim: %v", err)
	}
	fuzzTargets, err := GetFuzzTargets(c, fuzzParameters)
	if err != nil {
		return nil, fmt.Errorf(
			"could not get the fuzzing targets to generate the fuzzing claim: %v", err)
	}
	fuzzClaimSpec, err := generateFuzzClaimSpec(c, revisionDigest, fuzzParameters, fuzzTargets)
	if err != nil {
		return nil, fmt.Errorf(
			"could not get the fuzzing ClaimSpec to generate the fuzzing claim: %v", err)
	}
	evidences, err := GetEvidences(c, fuzzParameters, fuzzTargets)
	if err != nil {
		return nil, fmt.Errorf(
